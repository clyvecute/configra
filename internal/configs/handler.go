...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// Basic validation
	if req.ProjectID == 0 || req.EnvID == 0 || req.Key == "" {
		utils.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "missing required fields"})
		return
	}

	// Call Service
	// Hardcoding userID = 1 for now (admin)
	cfg, err := h.service.CreateConfig(req.ProjectID, req.EnvID, req.Key, req.Data, req.Schema, 1)
	if err != nil {
